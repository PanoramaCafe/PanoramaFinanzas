/* Panorama Core integration: Finanzas — consolidado */
(async()=>{
  ...
  async function bootstrapFinanceState(){
    try{
      ...
      }else if(localRaw){await syncNow(JSON.parse(localRaw));}
    ...
    try{
      ...
      if(form.id==='movementForm'){
        ...
        if(data.has('from')&&data.has('to')){
          const from=getAccount(state,data.get('from')),to=getAccount(state,data.get('to'));if(!from||!to||from.id===to.id){alert('Revisa origen, destino e importe.');return;}
          const before=Number(from.balance||0);from.balance=Number((before-amount).toFixed(2));to.balance=Number((Number(to.balance||0)+amount).toFixed(2));state.moves=Array.isArray(state.moves)?state.moves:[];state.moves.push({id:uid(),created:Date.now(),origin:'manual',type:'transferencia',date:data.get('date')||'',amount,concept:'Transferencia',category:'',from:from.id,to:to.id,account:from.id,note:data.get('note')||''});addNegativeObservation(state,from,before,amount,'Transferencia');writeState(state);await closeAndReload(state);return;
        }
        const acc=getAccount(state,data.get('account'));if(!acc){alert('Revisa la cuenta.');return;}
        const isOut=data.has('category'),before=Number(acc.balance||0);acc.balance=Number((before+(isOut?-amount:amount)).toFixed(2));state.moves=Array.isArray(state.moves)?state.moves:[];state.moves.push({id:uid(),created:Date.now(),origin:data.get('origin')||'manual',externalId:data.get('externalId')||'',type:isOut?'salida':'entrada',date:data.get('date')||'',amount,concept:data.get('note')||(isOut?'Salida manual':'Entrada manual'),category:data.get('category')||'',from:isOut?acc.id:null,to:isOut?null:acc.id,account:acc.id,note:data.get('note')||''});if(isOut)addNegativeObservation(state,acc,before,amount,'Salida manual');writeState(state);await closeAndReload(state);return;
      }
      if(form.id==='payrollPaymentForm'){
        const amount=Number(data.get('amount')),acc=getAccount(state,data.get('account')),employee=(state.payrollEmployees||[]).find(e=>String(e.id)===String(data.get('employee')));if(!Number.isFinite(amount)||amount<=0||!acc||!employee){alert('Revisa los datos.');return;}
        const before=Number(acc.balance||0),rec={id:uid(),created:Date.now(),employeeId:employee.id,employeeName:employee.name,amount,accountId:acc.id,date:data.get('date')||'',origin:data.get('origin')||'local',externalId:data.get('externalId')||''};acc.balance=Number((before-amount).toFixed(2));state.payrollPeriods=Array.isArray(state.payrollPeriods)?state.payrollPeriods:[];state.moves=Array.isArray(state.moves)?state.moves:[];state.payrollPeriods.push(rec);state.moves.push({id:uid(),created:Date.now(),origin:'nomina',externalId:rec.externalId,sourceRecordId:rec.id,type:'salida',date:rec.date,amount,concept:'Nómina — '+employee.name,category:'nomina',from:acc.id,to:null,account:acc.id,note:'Pago de nómina'});addNegativeObservation(state,acc,before,amount,'Pago de nómina');writeState(state);await closeAndReload(state);return;
      }
      const account=getAccount(state,data.get('account')),requestId=form.dataset.requestId;if(!account){alert('Selecciona una cuenta.');return;}
      const req=(await window.PanoramaCoreFinance.pending()).find(r=>String(r.id)===String(requestId));if(!req){alert('La solicitud ya no está pendiente.');return;}
